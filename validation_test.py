#!/usr/bin/env python3
"""
Comprehensive validation test script for MemOS quantized model
"""
import json
import time
import torch
...
class ModelValidator:
    """Comprehensive model validation with performance monitoring."""
    ...
    def load_quantized_model(self) -> bool:
        """Load the quantized model or create it if it doesn't exist."""
        try:
            print(f"📂 Checking for quantized model at: {self.quantized_path}")
            
            if not self.quantized_path.exists():
                print("⚠️  Quantized model not found, creating it...")
            ...
            # Use CPU for inference to avoid MPS issues
    ...
    def _create_quantized_model(self):
        ...
        # Import and use the Apple Silicon optimizer
        from quantize_apple_silicon import AppleSiliconOptimizer, OptimizationConfig
        
        config = OptimizationConfig(
            model_path=str(self.model_path),
            output_path=str(self.quantized_path)
        )
        
        optimizer = AppleSiliconOptimizer(config)
        # Run just the optimization without benchmarking to avoid MPS issues
    ...
    def run_single_test(self, test_case: Dict) -> ValidationResult:
        ...
        try:
            ...
            # Move to device (CPU for stability)
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs,
                    max_new_tokens=150,  # Limit output for testing
                    do_sample=True,
                    temperature=0.7,
    ...
    def generate_report(self, results: List[ValidationResult]) -> Dict:
        ...
        # Performance checks
        performance_checks = {
            "inference_latency_ok": max_inference_time < 200,  # <200ms requirement
    ...
    def save_results(self, filename: str = "validation/results.json"):
...
def main():
    ...
    print("🎯 MemOS Quantized Model Validation Suite")
    print("=" * 50)
