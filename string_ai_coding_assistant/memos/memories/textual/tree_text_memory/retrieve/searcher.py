...
from string_ai_coding_assistant.memos.graph_dbs.factory import Neo4jGraphDB
from string_ai_coding_assistant.memos.llms.factory import OllamaLLM, OpenAILLM
from string_ai_coding_assistant.memos.memories.textual.item import SearchedTreeNodeTextualMemoryMetadata, TextualMemoryItem
...
class Searcher:
    ...
    def search(
        ...
        # Always ensure query_embedding is defined as a list of embeddings
        if parsed_goal.memories:
            query_embedding = self.embedder.embed(list({query, *parsed_goal.memories}))
        else:
            # If no parsed memories, just embed the single query and wrap in list format
            query_embedding = [self.embedder.embed([query])[0]]
