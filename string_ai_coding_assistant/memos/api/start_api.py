...
@app.get("/health", summary="Health check endpoint", response_model=BaseResponse)
async def health_check():
    """
    Health check endpoint that verifies MemOS service and model status.
    
    Returns:
        BaseResponse: Health status with service and model information
    """
    try:
        # Check MemOS service status
        mos_instance = get_mos_instance()
        memos_healthy = mos_instance is not None
        
        # Initialize health data
        health_data = {
            "status": "healthy" if memos_healthy else "unhealthy",
            "memos_service": {
                "status": "running" if memos_healthy else "error",
                "user_id": mos_instance.user_id if memos_healthy else None,
                "cubes_count": len(mos_instance.mem_cubes) if memos_healthy else 0,
            },
            "model": {
        ...
        # Check model status if it's a GGUF model
        if memos_healthy and hasattr(mos_instance, 'chat_llm'):
            chat_llm = mos_instance.chat_llm
            if chat_llm and hasattr(chat_llm, 'is_healthy'):
                # GGUF model health check
                model_healthy = chat_llm.is_healthy()
                health_data["model"] = {
                    "status": "healthy" if model_healthy else "unhealthy",
                    "type": "gguf",
                    "loaded": model_healthy,
                    "info": chat_llm.get_model_info() if hasattr(chat_llm, 'get_model_info') else None,
                }
            elif chat_llm:
                # Other model types
                health_data["model"] = {
                    "status": "healthy",
                    "type": getattr(chat_llm, '__class__', {}).get('__name__', 'unknown'),
                    "loaded": True,
                    "info": None,
                }
        ...
        overall_healthy = (
            health_data["memos_service"]["status"] == "running" and
            health_data["model"]["status"] in ["healthy", "not_configured"]
        )
        ...
        return BaseResponse(
            code=response_code,
            message="Health check completed" if overall_healthy else "Service unhealthy",
            data=health_data
        )
        ...
        return BaseResponse(
            ...
            data={
                "status": "unhealthy",
                "error": str(e),
                "memos_service": {"status": "error"},
                "model": {"status": "error", "loaded": False}
            }
        )
