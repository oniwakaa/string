...
from string_ai_coding_assistant.memos.configs.llm import HFLLMConfig
...
class HFLLM(BaseLLM):
    ...
    def __init__(self, config: HFLLMConfig):
        ...
        # Standard model initialization without quantization
        ...
        # Initialize tokenizer (same path as model)
    ...
    def generate(self, messages: MessageList, past_key_values: DynamicCache | None = None):
        ...
        try:
            ...
            logger.warning(f"Chat template not available or failed: {e}")
            # Fallback to simple concatenation for models without chat templates
            prompt = self._create_simple_prompt(messages)
    ...
    def _create_simple_prompt(self, messages: MessageList) -> str:
        """
        Create a simple prompt from messages when no chat template is available.
        Args:
            messages (MessageList): Chat messages.
        Returns:
        ...
        if getattr(self.config, 'add_generation_prompt', True):
    ...
    def build_kv_cache(self, messages) -> DynamicCache:
        ...
        try:
            ...
            logger.warning(f"Chat template not available or failed in build_kv_cache: {e}")
