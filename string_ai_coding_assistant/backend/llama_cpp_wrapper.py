...
if MEMOS_AVAILABLE:
    ...
    class LlamaCppWrapper:
        """
        Fallback LlamaCpp wrapper when MemOS is not available.
        """
        
        def __init__(self, llama_instance: "Llama", config: Optional[Any] = None):
            """
            Initialize the wrapper with an existing llama-cpp-python Llama instance.
            
            Args:
                llama_instance (Llama): Pre-loaded llama-cpp-python Llama instance
                config (Optional[Any]): Configuration object (ignored when MemOS unavailable)
            """
            if not LLAMA_CPP_AVAILABLE:
                raise RuntimeError("llama-cpp-python is required but not available")
            ...
            logger.info("LlamaCpp fallback wrapper initialized successfully")
        ...
        def _messages_to_prompt(self, messages: list) -> str:
            """
            Convert MessageList format to a prompt string suitable for llama-cpp-python.
